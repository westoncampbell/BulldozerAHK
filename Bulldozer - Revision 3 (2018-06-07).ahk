...
BackgroundLoad(MapData) {
	...
	; Send image to static control
	GDI_SetImageX(hBitmap, hBackground)
...
GDI_SetImageX(hBM, hCtrl) {	; http://www.autohotkey.com/forum/viewtopic.php?p=488784#488784
	hdcSrc := DllCall("Gdi32.dll\CreateCompatibleDC", (A_PtrSize ? "UPtr" : "UInt"), 0)
	hdcDst := DllCall("User32.dll\GetDC", (A_PtrSize ? "UPtr" : "UInt"), hCtrl)
	VarSetCapacity(bm, 24, 0) ; BITMAP Structure
	DllCall("Gdi32.dll\GetObject", (A_PtrSize ? "UPtr" : "UInt"), hbm, "Int", 24, Ptr, &bm)
	w := Numget(bm, 4), h := Numget(bm, 8)
	hbmOld := DllCall("Gdi32.dll\SelectObject", (A_PtrSize ? "UPtr" : "UInt"), hdcSrc, (A_PtrSize ? "UPtr" : "UInt"), hBM)
	hbmNew := DllCall("Gdi32.dll\CreateBitmap", "Int", w, "Int", h, "UInt", NumGet(bm, 16, "UShort"), "UInt", NumGet(bm, 18, "UShort"), "Int", 0)
	hbmOld2 := DllCall("Gdi32.dll\SelectObject", (A_PtrSize ? "UPtr" : "UInt"), hdcDst, (A_PtrSize ? "UPtr" : "UInt"), hbmNew)
	DllCall("Gdi32.dll\BitBlt", "UInt", hdcDst, "Int", 0, "Int", 0, "Int", w, "Int", h, "UInt", hdcSrc, "Int", 0, "Int", 0, "UInt", 0x00CC0020)
	DllCall("Gdi32.dll\SelectObject", (A_PtrSize ? "UPtr" : "UInt"), hdcSrc, (A_PtrSize ? "UPtr" : "UInt"), hbmOld)
	DllCall("Gdi32.dll\DeleteDC", (A_PtrSize ? "UPtr" : "UInt"), hdcSrc)
	DllCall("User32.dll\ReleaseDC", (A_PtrSize ? "UPtr" : "UInt"), hCtrl, (A_PtrSize ? "UPtr" : "UInt"), hdcDst)
	DllCall("User32.dll\SendMessage", "UInt", hCtrl, "UInt", 0x0B, "UInt", 0, "UInt", 0) ; WM_SETREDRAW OFF
	oBM := DllCall("User32.dll\SendMessage", "UInt", hCtrl, "UInt", 0x172, "UInt", 0, "UInt", hBM) ; STM_SETIMAGE
	DllCall("User32.dll\SendMessage", "UInt", hCtrl, "UInt", 0x0B, "UInt", 1, "UInt", 0) ; WM_SETREDRAW ON
	DllCall("Gdi32.dll\DeleteObject", (A_PtrSize ? "UPtr" : "UInt"), oBM)
}
...
MenuHandler(ItemName, ItemPos, MenuName) {
	...
	If (MenuName = "GameMenu") {
		...
		If (ItemName = "&About Bulldozer...") {
			Gui, Main: +OwnDialogs
			MsgBox, 0x40, About Bulldozer..., Bulldozer`nRevision 3 - 2018-06-07`n`nBulldozer is a clone (with some differences) of [Bulldozer] from`n[24 Games for Windows 95] released by [Expert Software, Inc.]`n`nDeveloped for AutoHotkey by:`nWeston Campbell <westoncampbell@gmail.com>`n`nhttps://autohotkey.com/boards/viewtopic.php?f=6&t=50139
		}
	}
}
